...
public:
    ...
    NetworkResponse perform(const NetworkRequest& request) override {
        ...
        curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, request.strict_ssl ? 1L : 0L);
        curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, request.strict_ssl ? 2L : 0L);
        ...
        if (rc != CURLE_OK) {
            return {
                Error{ErrorCode::ERR_NETWORK_TIMEOUT, std::string("Network request failed: ") + curl_easy_strerror(rc)},
                http_code,
                {},
                {},
                cost,
        ...
        if (mapped != ErrorCode::OK) {
            return {
                Error{mapped, std::string("HTTP status code abnormal: ") + std::to_string(http_code)},
                http_code,
                {},
                {},
                cost,
