...
int main() {
    ...
    // 异步接口：应可正常返回结果。
    auto future_decision = chronosstream::PlaybackFacade::resolveProgrammePlaybackAsync(m3u, ended, now, {}, false);
    const auto async_decision = future_decision.get();
    if (async_decision.mode != chronosstream::PlaybackMode::CATCHUP) return EXIT_FAILURE;
